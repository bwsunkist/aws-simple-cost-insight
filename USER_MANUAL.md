...
#### 5.7 **🆕 コスト詳細テーブル**
アカウント別・サービス別の月次コスト詳細を表形式で確認できる機能：

**表示方法**:
1. **対象アカウント選択**: 「全アカウント合計」または個別アカウントを選択
2. **自動表示**: 選択後、即座にテーブルが更新表示

**テーブル構成**:
- **基本列**: サービス | 月次データ | 合計 | **🆕 前月比**
- **月次データ**: 各月の実際のコスト（例：2025/01、2025/02...）
- **合計列**: 全期間の合計コスト
- **🆕 前月比列**: 最新月と前月の差分を表示（合計欄の右側）
...
- **表示形式**: 
  ...
  - 変化なし: +$0（ニュートラル表示）
- **適用範囲**: 各サービス行 + 合計行の両方
- **データ要件**: 最低2ヶ月のデータが必要（1ヶ月のみの場合は「-」表示）

**アコーディオン機能**:
- **サービス名クリック**: サービス名をクリックで詳細な前月比情報を展開
- **詳細表示**: 各月の前月比差分と前月比率（%）を表示
- **一括操作**: 
  - 「全て展開」: 全サービスの詳細を一括表示
  - 「全て折りたたみ」: 全詳細を一括非表示
...
**活用例**:
```
サービス | 2025/01 | 2025/02 | 2025/03 | 合計  | 前月比
---------|---------|---------|---------|-------|---------
S3       | $42     | $40     | $42     | $124  | +$3
EC2      | $32     | $29     | $12     | $73   | -$8
Lambda   | $22     | $29     | $12     | $63   | -$17
合計     | $96     | $98     | $66     | $260  | -$32
```

**利用シーン**:
- **月次変化の詳細把握**: 各サービスの前月からの具体的な変化額を確認
- **トレンド分析**: 増加・減少トレンドの早期発見
- **予算管理**: 月次変化を踏まえた次月予算の調整検討
- **最適化効果測定**: コスト削減施策の即座の効果確認
- **異常検知**: 想定外の大幅コスト変化の迅速な特定
