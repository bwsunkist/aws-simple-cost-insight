...
### #007: サービス数UI問題 - 「その他」が上位Nサービス数に含まれる
- **発見日**: 2025-07-22
- **重要度**: High
- **ステータス**: Fixed
- **発見者**: ユーザーフィードバック
...
#### 症状
- **発生画面**: アカウント間構成比比較グラフ
- **期待動作**: 「上位3サービス」選択時、3つのサービス + その他 = 4つの項目表示
- **実際動作**: 「その他」を含めて3つしか表示されない（EC2, S3, その他のみ）
- **具体的な問題**: ユーザーが期待する3つのサービスが表示されない

#### 再現手順
1. 複数アカウントのデータを登録（test-dev, test-prod）
2. データ分析開始
3. アカウント間構成比比較の「表示サービス数」を「上位3サービス」に設定
4. チャート凡例で項目数確認→期待4つ、実際3つ

#### 環境情報
- **ブラウザ**: Playwright (Chromium based)
- **データ**: 複数アカウント（dev: 6サービス, prod: 7サービス）
- **問題箇所**: `js/chart-config.js` の `createAccountComparisonChartConfig`関数

#### 影響範囲
- **ユーザビリティ**: 設定値と実際の表示が異なる混乱
- **データ視認性**: 重要なサービスが「その他」に隠れる可能性
- **UI一貫性**: 他のサービス数設定との不整合

#### 修正内容 (2025-07-22)
- **根本原因**: 色配列生成で `otherServices.length > 0` の条件分岐が原因
- **修正内容**:
  - `js/chart-config.js:937` - 色配列を `topServices.length + 1` で事前確保
  - `js/chart-config.js:956-971` - 「その他」を条件なしで常に追加
  - `js/chart-config.js:1017-1022` - 全体合計でも一貫して「その他」を追加
- **技術的改善**: UI一貫性の向上、条件分岐簡素化

#### E2E検証結果
- 「上位3サービス」選択: ✅ 4項目表示（EC2, S3, CloudWatch, その他）
- 設定変更時リアルタイム更新: ✅ 正常動作
- 100%積み上げ棒グラフ表示: ✅ 正常な構成比表示
- ユーザー期待値一致: ✅ 設定通りのサービス数表示
