...
#### 4.3 サービス別構成比（円グラフ）- 最新月データ
**🆕 更新**: 表示サービス数選択機能と「その他」グループ化機能を追加

**設定項目**:
- **アカウント選択**: ドロップダウンで「全アカウント」または個別アカウントを選択
- **表示サービス数**: 
  - 上位3サービス: トップ3 + その他
...
**グラフの見方**:
- **データ基準**: 最新月（直近の月）のコストデータ
- **構成比表示**: 各サービスの割合を円グラフで可視化
- **詳細情報**: 各セクションにサービス名と割合（%）を表示
- **「その他」**: 選択したサービス数を超える残りサービスをグレー色で集約表示
- **色分け**: 上位サービスは明確な色、「その他」はグレーで区別

**活用方法**: 
- サービス利用の偏りチェック、コスト構造の理解
- 多数のサービス使用時でも見やすい円グラフで重要サービスに注目
- 積み上げグラフと同じ設定で一貫した分析
...
#### 4.5 **🆕 アカウント間構成比比較（最新月）- 100%積み上げ棒グラフ**
...
**グラフの見方**:
- **チャート形式**: 100%積み上げ棒グラフで全アカウントを一画面で比較表示
- **X軸**: 各アカウント名
- **Y軸**: 構成比（0-100%）
- **データ基準**: 各アカウントの最新月コストデータ
- **構成比表示**: 各アカウントの棒グラフで各サービスの割合を可視化
- **統一設定**: 全アカウントで同じ表示サービス数設定を適用
...
**活用シナリオ**:
- **構成比の差異分析**: アカウント間でのサービス利用パターンの違いを横並び比較で一目で把握
- **サービス別傾向**: 特定サービスが全アカウントでどの程度の比重を占めるか縦方向で確認
- **標準化機会の発見**: 構成比が近いアカウントを特定して統一化の検討
- **最適化対象の優先度決定**: 高比重サービスと低比重サービスの判別でコスト削減焦点を絞り込み
- **アーキテクチャ比較**: 開発・ステージング・本番環境での構成比較分析
- **予算配分の検討**: アカウント特性に応じた予算配分戦略の立案
...
**表示内容**:
- **テーブル形式**: アカウント、基準コスト、最新コスト、削減額、削減率、効果評価
- **削減効果アイコン**:
  - 🎯 大幅削減（25%以上）
  - ✅ 効果的削減（5-25%）
  - 📈 コスト増加
...
**活用例**（最高月比較の場合）:
```
dev: $135.5000 → $86.7500 (+$48.7500, +36.0%) 🎯
staging: $210.8000 → $199.2000 (+$11.6000, +5.5%) ✅
```
...
**設定方法**:
1. **開始月**: 月選択フィールドで比較開始月を設定
2. **終了月**: 月選択フィールドで比較終了月を設定
3. **実行**: 「期間比較実行」ボタンをクリック

**分析内容**:
- **期間内総コスト**: 指定期間でのアカウント別総コスト
- **月平均コスト**: 期間内の月平均コスト
- **可視化**: 混合チャート（棒グラフ+線グラフ）で表示
